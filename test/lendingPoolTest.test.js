    ...
    describe("LendingTracker", () => {
        describe("Deploys the new pool contracts", () => {
            ...
            it("Adds pool to a mapping", async () => {
                const mappingResult = await lendingTracker.tokenToPool(simpleToken.target)
            ...
            it("Pool variables", async () => {
                ...
                expect(await lendingPoolContract.ownerContract()).to.equal(lendingTracker.target)
            ...
            it("ChangePriceFeed", async () => {
                await lendingTracker.changePriceFeed(simpleToken.target, simpleToken.target)
                const mappingResult = await lendingTracker.tokenToPool(simpleToken.target)
        ...
        describe("lendToken", () => {
            beforeEach(async () => {
                ...
                await lendingTracker.lendToken(simpleToken.target, ethers.parseEther("500"))
            })
            it("Reverts if token not available", async () => {
                await expect(lendingTracker.lendToken(deployer, ethers.parseEther("10"))).to.be
                    .reverted
            })
            it("Updates the balances", async () => {
                ...
                expect(await simpleToken.balanceOf(lendingPoolContract.target)).to.equal(
                    ethers.parseEther("500")
                )
                expect(await simpleToken.balanceOf(lendingTracker.target)).to.equal(
                    ethers.parseEther("0")
                )
                expect(await simpleToken.balanceOf(deployer)).to.equal(ethers.parseEther("500"))
            })
            it("Updates the mapping", async () => {
                ...
                expect(await lendingTracker.userLended(deployer, simpleToken.target)).to.equal(
                    ethers.parseEther("500")
                )
            })
            it("Updates reserve in the pool", async () => {
                ...
                expect(await lendingPoolContract.reserve()).to.equal(ethers.parseEther("500"))
            })
        ...
        describe("Collateral", async () => {
            it("Reverts if pool doesnt exist", async () => {
                await simpleToken.approve(lendingTracker.target, mintAmount)
                await expect(lendingTracker.stakeCollateral(simpleToken.target, mintAmount)).to.be
                    .reverted
            })
            it("Stakes the collateral", async () => {
                ...
                expect(await simpleToken.balanceOf(lendingTracker.target)).to.equal(mintAmount)
                expect(await lendingTracker.collateral(deployer, simpleToken.target)).to.equal(
                    mintAmount
                )
            ...
            it("unstakes the collateral", async () => {
                ...
                expect(await simpleToken.balanceOf(lendingTracker.target)).to.equal("0")
                expect(await simpleToken.balanceOf(deployer)).to.equal(mintAmount)
                expect(await lendingTracker.collateralTokens(deployer, 0)).to.be.reverted
                expect(await lendingTracker.collateral(deployer, simpleToken.target)).to.equal("0")
            })
        ...
        describe("Borrow", () => {
            ...
            it("Reverts it pool is not available", async () => {
                await expect(lendingTracker.borrowToken(deployer, ethers.parseEther("10"))).to.be
                    .reverted
            })
